...
public class Function
{
    ...
    private async Task<List<string>> GetPaxNamesAsync(SheetsService sheetsService, Region region)
    {
        ...
        var paxMembers = result.Values.Select(x => x.FirstOrDefault()?.ToString()).Where(x => !string.IsNullOrWhiteSpace(x)).Distinct().ToList();
    ...
    private async Task<TowerChallengeResponse> GetTowerChallengeAsync(SheetsService sheetsService)
    {
        try
        {
            ...
            var sitesRange = $"{sactownRegion.AosSheetName}!A2:G";
            ...
            var challengeSites = siteRows
                .Where(row => row.Count > 6 &&
                              ...
                              string.Equals(row[6]?.ToString()?.Trim(), "Y", StringComparison.OrdinalIgnoreCase))
